...
kamatera_cluster_create_worker_node() {
    ...
    SERVER_PATH="${5}"
    # optional prefix to prepend to the autogenerated node id
    NODE_PREFIX="${6}"
    ...
    (
        ...
        if [ -e "environments/${K8S_ENVIRONMENT_NAME}/.env" ]; then
            ...
            if [ "${DISABLE_NODE_TAG}" != "1" ]; then
                ...
                while ! kamatera_cluster_shell "${K8S_ENVIRONMENT_NAME}" kubectl label node "${NODE_PREFIX}${NODE_ID}" "kamateranode=true"; do
                    sleep 20
                    echo .
                done
            fi
...
kamatera_cluster_create_persistent_node() {
    ...
    (
        ...
        NODE_PREFIX="${K8S_ENVIRONMENT_NAME}-${NODE_LABEL}"
        DISABLE_NODE_TAG="1"
        ! kamatera_cluster_create_worker_node "${K8S_ENVIRONMENT_NAME}" "${CPU}" "${RAM}" "${DISK_SIZE_GB}" \
                                              "${SERVER_PATH}" "${NODE_PREFIX}" "${SERVER_PASSWORD}" "${DISABLE_NODE_TAG}" \
...
kamatera_cluster_create_storage_node() {
    ...
    (
        ...
        export SSHPASS=$(cat "environments/${K8S_ENVIRONMENT_NAME}/secret-${NODE_LABEL}-pass")
        echo "setting up ${NODE_LABEL} node..."
        sshpass -e ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@$IP -- "mkdir -p /opt/kamatera/storage"
...
if [ "${1} ${2}" == "auth login" ]; then
    ...
    ### cluster commands ###
        ...
        kamatera_cluster_create_worker_node "${4}" "${5}" "${6}" "${7}" "${8}" "${9}"
        exit $?

    elif [ "${1} ${2} ${3}" == "cluster loadbalancer install" ]; then
        [ -z "${4}" ] && usage "cluster loadbalancer install <ENVIRONMENT_NAME> [ENV_CONFIG]" && exit 1
        kamatera_cluster_create_loadbalancer_node "${4}" "2B" "2048" "20" "" "${5}"
        exit $?
        ...
        ( [ -z "${4}" ] || [ -z "${5}" ] ) \
            && usage "cluster storage install <ENVIRONMENT_NAME> <DISK_SIZE_GB>" && exit 1
